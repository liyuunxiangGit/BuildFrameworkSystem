...
@interface HMTextLayer : CATextLayer
@end
...
- (void)commonInit {
    ...
    //dot set
    self.dotContentInset = UIEdgeInsetsMake(2.5, 2.5, 2.5, 2.5);
    self.dotOffset       = CGPointMake(0, 0);
    self.dotBgColor      = [UIColor redColor];
    self.dotTextColor    = [UIColor whiteColor];
    self.dotFont         = 13.0;
    self.dotType         = HMSegmentedControlDotTypeLeft;
...
- (void)setSectionNumbers:(NSArray *)sectionNumbers
{
    if (_sectionNumbers != sectionNumbers)
    {
        _sectionNumbers = sectionNumbers;
    }
    [self setNeedsDisplay];
}
...
- (NSAttributedString *)attributedNumbreAtIndex:(NSUInteger)index
{
    NSString *title = [self stringNumberAtIndex:index];
    ...
    {
        NSDictionary *dict = @{
                               NSForegroundColorAttributeName : self.dotTextColor,
                               NSFontAttributeName            : [UIFont systemFontOfSize:self.dotFont],
                               };
...
- (NSString *)stringNumberAtIndex:(NSUInteger)index
{
    NSString *title = nil;
    if (index < self.sectionNumbers.count)
    {
        title = [NSString stringWithFormat:@"%ld", [self.sectionNumbers[index] integerValue]];
    }
...
- (CGSize)stringOfSizeWithText:(NSString *)text
{
    NSDictionary *attributes = @{
                                 NSFontAttributeName : [UIFont systemFontOfSize:self.dotFont],
                                 
                                 };
    CGRect rect = [text boundingRectWithSize:CGSizeMake(CGFLOAT_MAX, 15) options:(NSStringDrawingUsesLineFragmentOrigin) attributes:attributes context:nil];
    CGSize size = CGSizeMake(rect.size.width + self.dotContentInset.left + self.dotContentInset.right, rect.size.height + self.dotContentInset.top + self.dotContentInset.bottom);
...
- (void)drawRect:(CGRect)rect {
	...
	if (self.type == HMSegmentedControlTypeText)
    {
		[self.sectionTitles enumerateObjectsUsingBlock:^(id titleString, NSUInteger idx, BOOL *stop) {
            ...
            id number = [self attributedNumbreAtIndex:idx];
            if (number)
            {
                CGSize dotSize = [self stringOfSizeWithText:[self stringNumberAtIndex:idx]];
                CGFloat dotY = (self.bounds.size.height - dotSize.height) * 0.5;
                
                CGRect itemFrame = CGRectZero;
                if ( self.dotType == HMSegmentedControlDotTypeLeft )
                {
                    itemFrame = CGRectMake(CGRectGetMinX(rect) - dotSize.width - self.dotOffset.x,
                                           dotY + self.dotOffset.y,
                                           dotSize.width,
                                           dotSize.height);
                }
                else if ( self.dotType == HMSegmentedControlDotTypeRight )
                {
                    itemFrame = CGRectMake(CGRectGetMaxX(rect) + self.dotOffset.x,
                                           dotY + self.dotOffset.y,
                                           dotSize.width,
                                           dotSize.height);
                }
                
                HMTextLayer *itemLayer      = [HMTextLayer layer];
                itemLayer.backgroundColor   = self.dotBgColor.CGColor;
                itemLayer.frame             = itemFrame;
                itemLayer.string            = number;
                itemLayer.cornerRadius      = dotSize.height * 0.5;
                itemLayer.masksToBounds     = YES;
                itemLayer.fontSize          = self.dotFont;
                if ([UIDevice currentDevice].systemVersion.floatValue < 10.0)
                {
                    //截断
                    titleLayer.truncationMode = kCATruncationEnd;
                }
                itemLayer.contentsScale = [[UIScreen mainScreen] scale];
                itemLayer.alignmentMode = kCAAlignmentCenter;
